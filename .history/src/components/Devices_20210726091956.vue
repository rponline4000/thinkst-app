      ...
      <v-expansion-panel-content>
        ...
        <v-timeline dense>
          <v-slide-x-reverse-transition group hide-on-leave>
            <v-timeline-item
              v-for="item in device_list.alerts"
              :key="item.local_time"
              :color="green"
              small
              fill-dot
  ...
  methods: {
    fetchData() {
      ...
      fetch(
        ...
        .then(() => {
          this.device_list = this.filterByNodeId(this.alerts, this.device_list);
        });
    },
    filterByNodeId(a1, a2) {
      let op = this.device_list.map((e, i) => {
        let temp = this.alerts.find((element) => element.node === e.node);
        if (temp.node_id) {
          e.node_id = temp.node_id;
        }
        return e;
      });
    },
