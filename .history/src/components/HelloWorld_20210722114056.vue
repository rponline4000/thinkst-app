  ...
  methods: {
    fetchData() {
      ...
      fetch(
        ...
        .then((data) => (this.device_list = data.device_list, this.alerts = this.filter(data.alerts, data.alerts.node_id === data.alerts.node_id)))
        
    },
    filter(arr, query) {
      return arr.filter(el => el.indexOf(query));
    }
  },
};
</script>
