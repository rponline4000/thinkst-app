  ...
  methods: {
    fetchData() {
      ...
      fetch(
        ...
        .then(() => {
          this.filterByNodeId(this.device_list);
        });
    },
    filterByNodeId() {
      this.device_list.map((device) => {
      let match = this.alerts.find(alert => alert.node_id == device.node_id);
       
          this.device_list.alerts.push(match)
