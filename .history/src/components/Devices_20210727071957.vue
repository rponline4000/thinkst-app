  ...
  methods: {
    fetchData() {
      ...
      fetch(
        ...
        .then(
          (data) => (
            (this.device_list = data.device_list), (this.alerts = this.filterByNodeId())
          )
        ).then(() => {this.filterByNodeId()});
    },

    filterByNodeId() {

     for (let index = 0; index < this.device_list.length; index++){
       const found = this.alerts.find(alert => alert.node_id === this.device_list[index].node_id)
       console.log(found)
       this.device_list[index]['alerts'] = Array(found)  
       console.log(this.device_list[index]['alerts'])
     }
    }
  },
};
