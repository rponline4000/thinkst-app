        ...
        <v-timeline dense>
          <v-slide-x-reverse-transition group hide-on-leave>
            ...
            >
              <v-row class="pt-1">
                <v-col cols="1">
                  <strong>{{ item.alerts[i].local_time }}</strong>
                </v-col>
                <v-col>
                  <strong>
                    <v-icon color="error" v-bind="attrs" v-on="on">
                    ...
                    <template v-slot:default>
                      <thead>
                        <tr>
                          ...
                          <th class="text-left">src_host</th>
                          <th class="text-left">src_host</th>
                      
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-for="(item, i) in device_list" :key="i">
                          <td class="text-left">{{ item.alerts[i].key }}</td>
                          <td class="text-left">{{ item.alerts[i].description }}</td>
                          <td class="text-left">{{ item.alerts[i].dst_host }}</td>
                          <td class="text-left">{{ item.alerts[i].src_host }}</td>
                          <td class="text-left">{{ item.alerts[i].created }}</td>
